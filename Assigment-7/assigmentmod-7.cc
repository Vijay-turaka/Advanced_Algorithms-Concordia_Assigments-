...
typedef struct {
	// Intializing user define data types 
	uchar data[64];
	uint datalen;
	uint bitlen[2];
	uint state[8];
...
int main()
{
    char data[] = "hello i am abc this is sha256 encryption";
	...
	// (first 32 bits of the fractional parts of the square roots of the first 8 primes 2..19): to state array
	pointer->datalen = 0;
	pointer->bitlen[0] = 0;
	pointer->bitlen[1] = 0;
